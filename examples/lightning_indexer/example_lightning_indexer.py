                    ...
                    for g in T.serial(G):
                        for m in T.serial(S1 // BLOCK_M):
                            for n in T.serial(S2 // BLOCK_N):
                                T.barrier_all()
                                T.copy(Query[cid, m * BLOCK_M, n2, g * D], Q_L1)
                                T.barrier_all()
                                T.copy(KEY[cid, n * BLOCK_N, n2, 0], K_L1)
                                T.barrier_all()
                                T.gemm_v0(Q_L1, K_L1, C_L0, transpose_B=True, init=True)
                                T.barrier_all()
                                T.copy(
                                    C_L0,
                                    QK_RES[cid, n2, m * BLOCK_M, n * BLOCK_N + g * S2],
                                    srcN=-1,
                                    enable_relu=True)
                                T.barrier_all()
                ...
                for s1_id in T.serial(s1_start_idx, s1_end_idx):
                    ...
                    for s2_id in T.serial(S2 // VECTOR_BASEN):
                        ...
                        for g_id in T.serial(G // VECTOR_BASEG):
                            T.barrier_all()
                            T.copy(
                                QK_RES[cid, n2_id, s1_id,
                                       g_id * VECTOR_BASEG * S2 + s2_id * VECTOR_BASEN], mm_res_ub,
                                S2)
                            T.barrier_all()
                            T.copy(WEIGHTS[cid, s1_id, n2_id, g_id * VECTOR_BASEG], weight_ub)
                            T.barrier_all()
